...
def build_model(vocab, tgt, dist_bar, N=6, embed_dim=args.embed_dim, ffn_dim=args.fnn_dim, head=4, dropout=args.dropout, out_both=False):
    ...
    position = PositionalEncoding3D_new(embed_dim, dropout)
...
class Encoder3D(nn.Module):
    ...
    def forward_once(self, src, src_mask, pos, dist):
    ...
    def forward(self, src, src_mask, pos, dist):
        if self.out_both:
            h = self.forward_once(src, src_mask, pos,dist)
            return self.generator(h), h

        return self.generator(self.forward_once(src, src_mask, pos, dist))
