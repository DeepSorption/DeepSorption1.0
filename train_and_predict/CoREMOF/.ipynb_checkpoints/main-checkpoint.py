...
def main():
    args = parse_args()
    log = Logger('../' + args.save_path + 'coremof/', f'msa_{strftime("%Y-%m-%d_%H-%M-%S", localtime())}.log')
    set_seed(args.seed)
    # args.batch_size = 32
    args.dist_bar = [[5, 8, 12, 1e10]]
    ...
    # data_y = torch.cat((data_y, data_gas), dim=1)[:, tgt]   # [10066, 7]
    ...
    data_all_train=torch.load('../pre_data/coremof/coremof_train.pt')
    data_all_val=torch.load('../pre_data/coremof/coremof_val.pt')
    data_all_test=torch.load('../pre_data/coremof/coremof_test.pt')
    ...
    checkpoint = {'model': best_model.state_dict(), 'n_tgt': len(tgt), 'dist_bar': args.dist_bar,
                    'epochs': best_epoch, 'lr': optimizer.param_groups[0]['lr']}
    if len(args.gpu_id) > 1: checkpoint['model'] = best_model.module.state_dict()
    torch.save(checkpoint, '../' + args.save_path + 'coremof/' + 'Coremof_Tgt_{}_{}_{}.pt'.format(args.fnn_dim, args.lr, args.batch_size))
    log.logger.info('Save the best model as Coremof_Tgt_{}_{}_{}.pt'.format(args.fnn_dim, args.lr, args.batch_size))
