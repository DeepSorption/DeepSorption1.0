...
def main():
    ...
    args = parse_args()
    args.save_path='save/'
    log = Logger(args.save_path + 'coremof/', f'msa_{strftime("%Y-%m-%d_%H-%M-%S", localtime())}_seednum_{seednum}.log')
    set_seed(seednum)
    ...
    torch.save(checkpoint,  args.save_path + 'coremof/' + 'Coremof_layer6_Tgt_{}_batchsize{}_distbar{}_traug.pt'.format(len(tgt), args.batch_size,distbar))
